...
print(toxcast.columns!= 'smiles')

#removing nans from toxcast
toxcast_nosmiles = toxcast[toxcast.columns!= 'smiles']
toxcast_nonans = toxcast_nosmiles.notna().astype(float)
clintox_nonans = clintox.notna().astype(float)
print(toxcast_nonans)
print(clintox_nonans)

#error: Item wrong length 618 instead of 8597 when smiles column is separated from other values

#merging datasets together
merged_sets = pd.concat([toxcast_nonans, clintox_nonans], axis = 1)
print(merged_sets)
...
#define X and y
X = merged_sets['smiles']
y = merged_sets['CT_Tox']
#look at the original dataset for diff features
#smiles strings got deleted- fix that
# calculations for vd and fu are dependent on the features or based on known data of the drugs
